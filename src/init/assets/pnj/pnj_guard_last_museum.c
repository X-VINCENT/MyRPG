...
char *message_guard_last[] = {
    "Today is a great day !",
    "Thieves are rare",
    NULL
};

void init_text_guard_last(pnj_t *pnj, sfVector2f pos)
{
    int nbr_text = my_arrlen(message_guard_last);

    pnj->text_to_display = malloc(sizeof(sfText *) * nbr_text + 1);
    pnj->text_to_display[nbr_text] = NULL;
    for (int i = 0; message_guard_last[i] != NULL; i++)
        pnj->text_to_display[i] = create_text(FONT_TEXT_PNJ, sfWhite,
            13, message_guard_last[i]);
    pos.y -= 220;
    pos.x -= 395;
...
void init_pnj_guard_last_museum(game_t *game)
{
    ...
    pnj_t *pnj = assets->pnj[PNJ_GUARD_LAST_MUSEUM];
    sfTexture *texture = game->textures->guard_pnj;
    sfVector2f position = {465, 2354};
