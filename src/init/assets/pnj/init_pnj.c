...
void init_black_pnj(pnj_t *pnj, sfVector2f pos, char *first_message, bool text,
                        game_t *game)
{
    pnj->sprite = create_sprite(game->textures->black_pnj, (sfIntRect){0, 30, 45, 65},
...
void citizens_png(game_t *game)
{
    ...
    for (int i = PNJ_BLACK_THREE + 1; i < PNJ_NBR; i++) {
        default_pnj_position.x = 1500 + rand() % (2500 + 1 - 1000);;
        default_pnj_position.y = 1700 + rand() % (1730 + 1 - 1705);;
